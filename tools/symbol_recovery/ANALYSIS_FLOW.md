...
**新增功能**：针对高指令数负载的热点函数，客观识别函数所属的性能类型

**背景信息**：
- 这些函数在性能分析中被检出为「高指令数占比函数（热点函数）」
- **重要**："热点" ≠ "性能瓶颈"
- 有些函数是计算密集型算法核心（如 FFT、DCT、解码循环），负载高但合理

**分析目标**：客观识别函数所属的性能类型
- ✅ **计算热点（Expected Hotspot）**：高负载但必要、算法高效
- ⚠️ **潜在瓶颈（Optimization Opportunity）**：存在低效逻辑或可改进空间
...
**生成的 Prompt**（当指定 `--open-source-lib` 时）:

```
请分析以下多个 ARM64 反汇编函数，推断每个函数的功能和可能的函数名。

🔍 重要提示：这是一个基于开源库 {open_source_lib} 的定制版本（三方库）。
   该 SO 文件是基于 {open_source_lib} 开源项目进行定制开发的，函数实现可能与标准 {open_source_lib} 库相似。
   请利用您对 {open_source_lib} 开源库的知识，结合反编译代码的特征，
   直接根据 {open_source_lib} 库中常见的函数名和功能模式来推断函数名和功能。
   如果反编译代码的特征与 {open_source_lib} 库中的某个函数匹配，请优先使用该函数名。

   📌 SIMD 向量化指令识别：
   请仔细检查反汇编代码中是否使用了 SIMD（单指令多数据）向量化指令，如：
   - ARM NEON 指令：addv, fadd, fmul, fmla, ld1, st1, dup, zip1, zip2, uzp1, uzp2 等
   - ARM SVE 指令：add, fadd, fmul, ld1w, st1w, whilelt, ptrue 等
   - 其他向量化指令模式：批量数据处理、并行计算、矩阵运算等
   如果识别到 SIMD 向量化指令的使用，请在功能描述中明确说明该函数使用了向量化优化，
   并描述向量化的具体用途（如批量数据处理、并行计算、矩阵运算等）。

   📌 开源库功能相关性：
   在功能描述中，请结合 {open_source_lib} 开源库的典型功能和特性，
   说明该函数在 {open_source_lib} 库中的作用和定位，以及与其他 {open_source_lib} 函数的关联性。
   例如：该函数是 {open_source_lib} 中哪个模块/组件的核心功能，
   在 {open_source_lib} 的典型使用场景中扮演什么角色，
   与 {open_source_lib} 库中哪些常见函数或功能相关。

背景信息：
这些函数在性能分析中被检出为「高指令数占比函数（热点函数）」，
但请注意：
  "热点" ≠ "性能瓶颈"。有些函数是计算密集型算法核心（如 FFT、DCT、解码循环），负载高但合理。
...
请分别提供以下信息：
  1. 功能描述：函数的主要功能是什么（不要包含性能分析）
     - 如果识别到 SIMD 向量化指令，请明确说明使用了向量化优化及其用途
     - 请结合 {open_source_lib} 开源库的功能特性，说明该函数在库中的作用和定位
     - 描述该函数与 {open_source_lib} 库中其他功能的关联性和相关性
  2. 负载问题识别与优化建议（performance_analysis）：
     判断函数属于：
       - "计算热点（预期高负载）"
       - "潜在瓶颈（可优化）"
     并说明原因，包括但不限于以下因素：
  ...
  "functions": [
    {
      ...
      "performance_analysis": "负载问题识别与优化建议（中文，100-300字）：判断函数属于"计算热点（预期高负载）"或"潜在瓶颈（可优化）"，说明原因（算法复杂度、内存访问模式、I/O调用、函数调用频度等），若存在可优化方向，给出具体可行建议",
      "confidence": "高/中/低",
...
# 返回:
{
    "functions": [{
        ...
        "performance_analysis": "潜在瓶颈（可优化）。该函数包含深度递归调用、大量字符串比较和解析操作，以及频繁的内存分配。优化建议：1) 使用迭代替代递归，降低栈开销；2) 缓存字符串比较结果，减少重复计算；3) 预分配内存池，减少动态分配开销。",
        "confidence": "高",
...
5. **性能分析**: 针对高指令数负载的热点函数，客观识别函数所属的性能类型（计算热点 vs 潜在瓶颈），分析高指令数负载原因，并提供优化建议
