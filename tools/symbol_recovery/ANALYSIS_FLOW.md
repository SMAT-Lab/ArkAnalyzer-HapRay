...
### 8. Prompt 构建 (Prompt Construction)
...
请按以下 JSON 格式返回分析结果:
{
  "functions": [
    {
      "function_id": "func_1",
      "functionality": "详细的功能描述（中文，50-200字）",
...
3. **字符串精准提取**: 通过分析指令中的字符串引用，而不是简单扫描内存
4. **调用堆栈信息**: 从 perf.db 获取真实的调用关系，提供更多上下文
5. **批量分析**: 一次分析多个函数，提高效率
6. **长度限制**: 根据函数大小动态调整反编译代码长度，避免包含函数边界外的代码
