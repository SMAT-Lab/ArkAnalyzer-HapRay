...
class BatchLLMFunctionAnalyzer(LLMFunctionAnalyzer):
    ...
    def _build_batch_prompt(self, functions_data: list[dict[str, Any]], context: Optional[str] = None) -> str:
        ...
        for idx, func_data in enumerate(functions_data, 1):
            ...
            if decompiled:
                ...
                # 增加反编译代码显示长度（从 100 行增加到 1000 行）
                max_decompiled_lines = 1000
            ...
            if uncached_batch:
                try:
                    ...
                    if self.save_prompts:
                        self._save_batch_prompt(batch_prompt, uncached_batch, batch_num, total_batches)
                    ...
                    response = self.client.chat.completions.create(
                        ...
                        max_tokens=16000,  # 批量模式下需要更多 tokens（增加缓冲，避免 JSON 截断）
    ...
    def _parse_batch_response(self, response_text: str, functions_data: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # 先尝试匹配完整的 JSON 对象
            json_match = re.search(r'\{[^{}]*"functions"[^{}]*\[.*?\]\s*\}', response_text, re.DOTALL)
            if not json_match:
            ...
            logger.warning('响应中是否包含 func_3: %s', 'func_3' in response_text)
