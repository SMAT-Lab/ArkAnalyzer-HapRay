...
class BatchLLMFunctionAnalyzer(LLMFunctionAnalyzer):
    ...
    def _build_batch_prompt(self, functions_data: list[dict[str, Any]], context: Optional[str] = None) -> str:
        ...
        # 如果指定了开源库，添加重要提示
        if self.open_source_lib:
            prompt_parts.append(f'🔍 重要提示：这是一个基于开源库 {self.open_source_lib} 的定制版本（三方库）。')
            prompt_parts.append(f'   该 SO 文件是基于 {self.open_source_lib} 开源项目进行定制开发的，函数实现可能与标准 {self.open_source_lib} 库相似。')
            prompt_parts.append(f'   请利用您对 {self.open_source_lib} 开源库的知识，结合反编译代码的特征，')
            prompt_parts.append(f'   直接根据 {self.open_source_lib} 库中常见的函数名和功能模式来推断函数名和功能。')
            prompt_parts.append(f'   如果反编译代码的特征与 {self.open_source_lib} 库中的某个函数匹配，请优先使用该函数名。')
            prompt_parts.append('')
            ...
            prompt_parts.append(f'   在功能描述中，请结合 {self.open_source_lib} 开源库的典型功能和特性，')
            prompt_parts.append(f'   说明该函数在 {self.open_source_lib} 库中的作用和定位，以及与其他 {self.open_source_lib} 函数的关联性。')
            prompt_parts.append(f'   例如：该函数是 {self.open_source_lib} 中哪个模块/组件的核心功能，')
            prompt_parts.append(f'   在 {self.open_source_lib} 的典型使用场景中扮演什么角色，')
            prompt_parts.append(f'   与 {self.open_source_lib} 库中哪些常见函数或功能相关。')
            prompt_parts.append('')
        ...
        if self.open_source_lib:
            ...
            prompt_parts.append(f'     - 请结合 {self.open_source_lib} 开源库的功能特性，说明该函数在库中的作用和定位')
            prompt_parts.append(f'     - 描述该函数与 {self.open_source_lib} 库中其他功能的关联性和相关性')
        else:
            prompt_parts.append('  1. 功能描述：函数的主要功能是什么（不要包含性能分析）')
        ...
        if self.open_source_lib:
            ...
            prompt_parts.append('   如果识别到向量NEON指令（v0-v31寄存器且有向量后缀），必须说明使用了向量化优化及其用途。')
            prompt_parts.append('   如果只使用标量浮点指令（如fmadd/fnmsub标量版本），不要误判为向量NEON，可以说明使用了融合乘加等标量优化指令。')
        prompt_parts.append('6. 负载问题识别与优化建议（performance_analysis）必须客观分析：')
