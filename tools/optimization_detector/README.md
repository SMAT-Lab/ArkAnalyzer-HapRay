...
- 每次运行结束后，默认在**主报告 `-o` 所在目录**（与 `dirname(abs(-o))` 一致）额外写入 **`hapray-tool-result.json`**（macOS 下若将输出映射到用户目录，则与映射后的报告同目录）。**`--result-file <path>`** 可显式指定契约文件路径（ArkAnalyzer-HapRay 桌面端 `tool_contract` 会注入该参数）。
