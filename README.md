...
- `--memory`: Enable Native Memory profiling using hiprofiler nativehook plugin
...
# Native Memory profiling (memory only)
...
**Native Memory Collection Modes:**
1. **Memory Only**: Use `--memory --no-trace --no-perf` to collect only Native Memory data
...
The Native Memory profiling uses HarmonyOS hiprofiler_cmd with nativehook plugin to collect:
