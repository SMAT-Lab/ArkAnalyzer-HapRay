...
| 2 | **确定 `tag`**：用户整链 URL 中的 tag → 否则 `HAPRAY_RELEASE_TAG` → 否则 Skill YAML `version` → `v{version}`（如 `1.5.9` → `v1.5.9`） |
| 3 | **确定 `version`**：`tag` 去掉前缀 `v`（`v1.5.9` → `1.5.9`），代入 `asset_name` 模板 |
...
**直链示例**（`tag=v1.5.9`，`version=1.5.9`）：

- Windows：`https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases/download/v1.5.9/ArkAnalyzer-HapRay-win32-x64-1.5.9.zip`
- Ubuntu 24.04：`.../ArkAnalyzer-HapRay-linux-x64-ubuntu24.04-1.5.9.zip`
- macOS Apple Silicon：`.../ArkAnalyzer-HapRay_1.5.9_aarch64.dmg`
- macOS Intel：`.../ArkAnalyzer-HapRay_1.5.9_x64.dmg`
