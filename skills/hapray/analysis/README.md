...
| `llm-high-load` | [`llm-high-load-analysis.md`](llm-high-load-analysis.md) | **已采集** perf/trace/log 等数据时，基于 **原始侧** 挖掘未知问题；**不用** `summary.json` 规则化摘要作主线 | 高负载特征、`trace.db`+`hiperf`+日志、可选与 HTML 对照、独立 `.md`「新发现」；**禁止**用 summary 替代原始挖掘 |
...
1. 在本目录新增 `*.md`（建议 `kebab-case`；专题分析类与现有文件一致，使用 **`*-analysis.md`** 后缀，如 `scroll-jank-trace-analysis.md`、`llm-high-load-analysis.md`、`symbol-recovery-analysis.md`）。
