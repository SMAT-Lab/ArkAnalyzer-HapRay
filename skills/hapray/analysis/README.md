...
| `symbol-recovery` | [`symbol-recovery-analysis.md`](symbol-recovery-analysis.md) | 报告或 **`perf.data`** 中热点为 **`libxxx.so+0x…`** / strip 后无可读符号、需还原函数名以定位瓶颈 | **SymRecover**（`tools/symbol_recovery`）、Radare2/r2dec、LLM 辅助；支持在线直连、主 Agent 离线编排、`--no-llm` 快速模式 |
