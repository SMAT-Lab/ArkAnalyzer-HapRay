...
| `symbol-recovery` | [`symbol-recovery-analysis.md`](symbol-recovery-analysis.md) | 同上；**新 clone 未构建**时先读 **`../SKILL.md` 源码硬门禁**与本文件 **§〇**，否则 Step1/perf→db 会先挂 | **SymRecover**（`tools/symbol_recovery` venv）；Radare2/r2dec **建议**；LLM 辅助；在线/离线/`--no-llm` |
| `empty-frame` | [`empty-frame-root-cause.md`](empty-frame-root-cause.md) | **空刷根因定位**、VSync 无效刷新、代码级修复建议；**update 已默认集成**（Agent + 拉 HAP） | 独立 `hapray root-cause` 或 update 产物 `report/root_cause.md`；`more.root_cause` 写入总报告；Agent 任务 `root_cause_agent_task.json` |
