...
1. **先判定运行轨（二选一，勿混用）**：  
   - **源码轨 `<REPO_ROOT>`**：存在 `perf_testing/pyproject.toml` 等 → **仅适用**下方「源码工作区硬门禁」；**先于任何** `perf` / `update` / dbtools / 符号恢复跑通 7 步构建自检。  
...
1. **进入「源码轨」时必须执行全部7步验证**，不可假设「以前配置过」或「应该没问题」；**二进制轨**不做本 7 步，改做「二进制发布包模式」最小自检。
...
1. **发布元数据获取能力**（满足其一即可，**不要求**必须能打开 GitCode 网页；**主要服务二进制轨下载**，源码轨可跳过下载）：  
   - **首选**：可访问 `https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases` 或 `.../releases/latest`（跟随重定向解析 tag）。  
   - **若 GitCode 网页/整站不可达**（超时、DNS、企业网屏蔽等）：**禁止**依赖浏览器或人工打开 releases 页；必须立即改用 **下文「§1.0 预设直链与镜像」**（环境变量 / Skill 表内备用根 / 用户直链），再无法下载则进入源码回退。  
2. 已确定当前平台与架构（Windows / Linux Ubuntu 22.04/24.04 x64 / macOS Intel / macOS Apple Silicon）。  
3. 已下载并解压**最新 release**（或 §1.0 锚定 tag）的对应平台二进制到 `<RUNTIME_ROOT>`，或在二进制不可用时已准备源码目录 `<REPO_ROOT>`。  
4. 真机链路可用（`hdc` 可执行且设备在线，若为真机场景）。  
5. 目标场景所需门禁已满足（如 `GLM_API_KEY`、symbol-recovery API Key）。

### 1.0) GitCode 不可达：预设直链与镜像（优先于“打开网页找包”）

当 **无法访问** GitCode 发布页或 `releases/latest` 时，按 **严格顺序** 取下载 URL，**先成功者采用**；全程须在执行轨迹记录 `download_url` 与 `url_source`。

| 优先级 | 来源 | 用法 |
|:------:|------|------|
| P0 | 用户/调用方给出的 **整条** `.../releases/download/<tag>/<asset_name>` 直链 | 直接使用，失败再降 P1。 |
| P1 | 环境变量 **`HAPRAY_RELEASES_DOWNLOAD_BASE`** | 值为 **站点根**，须以 `/` 结尾、**不含** `releases/download` 段。拼接：`${HAPRAY_RELEASES_DOWNLOAD_BASE}releases/download/${tag}/${asset_name}`。示例：`export HAPRAY_RELEASES_DOWNLOAD_BASE=https://your-mirror.example.com/SMAT/ArkAnalyzer-HapRay/` |
| P2 | **Skill 内置备用根**（与 GitCode **同路径后缀**，仅换主机；由团队维护 Release 镜像时填写） | 按下表「备用下载根」自上而下尝试；每个根与「§自动推断」中的 `tag`、`asset_name` 规则拼接为完整直链并做 GET 校验。 |

**备用下载根（P2，维护者随 Release 更新镜像后同步改 URL）**

> 默认与官方仓库路径一致：路径段均为 `SMAT/ArkAnalyzer-HapRay/releases/download/`。若某行主机不可达，Agent 应跳过该行试下一行。  
> **若 GitCode 域名级不可达**（DNS/超时）：跳过下表序号 1，仅从序号 2 及以后尝试，或仅依赖 **P1** `HAPRAY_RELEASES_DOWNLOAD_BASE`。

| 序号 | 备用根 `BASE`（末尾须带 `/`） | 说明 |
|:----:|-------------------------------|------|
| 1 | `https://gitcode.com/SMAT/ArkAnalyzer-HapRay/` | 与官方同源；若仅「网页 blocked」但 API/直链仍通，可配合代理使用。 |
| 2 | `https://<MIRROR_HOST>/SMAT/ArkAnalyzer-HapRay/` | **由团队填写**：与官方相同的 `releases/download/<tag>/<asset>` 路径结构；无镜像则删除本行。 |

**无法解析 `latest` / release 列表时的锚定 tag（最后手段）**

仅当 **(a)** P0–P2 均无法先取得可信 `tag`，**且** **(b)** 用户未提供 `tag` 时，允许使用 **本 Skill 正文顶部 `version` 字段** 对应的 tag（例如 `version: "1.5.4"` → `v1.5.4`）与「标准命名约定」拼接直链，在 P1/P2 的 BASE 上尝试下载。执行轨迹**必须**包含：`fallback_tag_source=skill_version`。

**与后文规则的关系**：§「自动推断下载直链」中凡依赖访问 `gitcode.com` 页面或 `releases/latest` 的步骤，在判定 GitCode 不可达后 **改为**仅使用 P0→P1→P2 + 锚定 tag，**不得**无限重试网页。

### 1) 二进制获取（必须下载最新 release）

默认从发布页下载（在 GitCode 可达时）：

`https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases`

**若 GitCode 不可达**，跳过本节中依赖网页列表的步骤，直接执行 **§1.0** 后再用已确定的 `tag` + 平台 `asset_name` 拼接直链。
...
1. 发布详情页形态：`https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases/<tag>`（示例：`.../releases/v1.5.4`）。  
2. 资产下载直链形态：`https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases/download/<tag>/<asset_name>`。  
3. `tag` 必须来自“最新 release”的真实 tag（如 `v1.5.4`），禁止写死版本。  
4. `asset_name` 必须来自该 release 的真实附件名，禁止臆造。  
5. 若已知 Windows 命名规则为 `ArkAnalyzer-HapRay-win32-x64-<version>.zip`，则可按模板拼接：
   - `version = tag` 去掉前缀 `v`（例如 `v1.5.4 -> 1.5.4`）
   - `asset_name = ArkAnalyzer-HapRay-win32-x64-<version>.zip`
   - 示例直链：`https://gitcode.com/SMAT/ArkAnalyzer-HapRay/releases/download/v1.5.4/ArkAnalyzer-HapRay-win32-x64-1.5.4.zip`
...
**若已判定 GitCode 不可达**：跳过本节中依赖访问 `gitcode.com` 网页或 `releases/latest` 的步骤，改按 **§1.0**（P0→P1→P2 + 锚定 tag）构造 URL 并下载；本节后续「候选直链」「附件列表」逻辑仅在 **能从镜像或用户取得 tag 与 asset 名** 时继续适用。
