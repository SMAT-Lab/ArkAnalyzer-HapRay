...
// 独立仓库：build.bun.ts 位于仓库根，node_modules 也在仓库根。
// 旧 monorepo 里 __dirname 是 tools/static_analyzer，需 ../.. 回到根；独立仓无需上溯。
...
// 跨平台编译目标，例如 bun-darwin-arm64 / bun-darwin-x64 / bun-linux-x64 / bun-windows-x64。
// 留空则按宿主平台编译（target: "bun"）。
const targetArg = (() => {
  const a = process.argv.find((x) => x.startsWith("--target="));
  return a ? a.slice("--target=".length) : "";
})();

function targetExt(target: string): string {
  // 交叉编译时不能依赖 process.platform 判定后缀，需看 target triple
  if (target.includes("windows")) return ".exe";
  if (!target) return process.platform === "win32" ? ".exe" : "";
  return "";
}

async function buildBinary() {
  console.log(
    `Building static_analyzer binary with Bun${targetArg ? ` (target=${targetArg})` : ""}...`
  );
