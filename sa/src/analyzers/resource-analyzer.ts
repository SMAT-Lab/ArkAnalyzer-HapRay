...
import { FileType, ResourceFileInfo, ResourceAnalysisResult, ArchiveFileInfo, JsFileInfo, HermesFileInfo } from '../types';
...
export class ResourceAnalyzer {
    ...
    constructor(fileSizeLimits: FileSizeLimits = DEFAULT_FILE_SIZE_LIMITS, maxRecursionDepth: number = 5) {
    ...
    public async analyzeResourcesFromZip(zip: ZipInstance): Promise<ResourceAnalysisResult> {
        ...
        const analysisContext: AnalysisContext = {
            filesByType: new Map<FileType, ResourceFileInfo[]>(),
            archiveFiles: [],
            jsFiles: [],
            hermesFiles: [],
            totalSize: 0,
     ...
     * 将文件添加到类型映射中
     */
    private addFileToTypeMap(filesByType: Map<FileType, ResourceFileInfo[]>, fileInfo: ResourceFileInfo): void {
