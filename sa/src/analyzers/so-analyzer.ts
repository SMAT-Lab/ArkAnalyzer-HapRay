...
import { FrameworkTypeKey, SoAnalysisResult } from '../types';
...
export class SoAnalyzer {
    ...
    private readonly supportedArchitectures: string[];
    ...
    public async analyzeSoFilesFromZip(zip: ZipInstance): Promise<{
        ...
        const soFiles: SoAnalysisResult[] = [];
     ...
     * @returns 识别到的框架类型数组
     */
    private identifyFrameworks(fileName: string): FrameworkTypeKey[] {
        try {
            // 首先检查是否是系统库
