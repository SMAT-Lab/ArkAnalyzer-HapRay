import os, re, json, argparse, logging, random, csv, struct, shutil, subprocess, math
...
def _safe_split_counts(n:int, ratios) -> Tuple[int,int,int]:
    r = np.array(ratios, dtype=float); r = r / r.sum()
    n_tr = int(np.floor(n * r[0])); n_va = int(np.floor(n * r[1])); n_te = n - n_tr - n_va
    if n >= 3 and n_te == 0:
        if n_va > 1: n_va -= 1; n_te += 1
        elif n_tr > 1: n_tr -= 1; n_te += 1
    if n >= 2 and n_va == 0 and (n_tr > 1):
        n_tr -= 1; n_va += 1
    if n_tr + n_va + n_te != n:
        diff = n - (n_tr + n_va + n_te); n_tr += diff
    return n_tr, n_va, n_te

def stratified_split(items:List[Tuple[str,int]], ratios=(0.6,0.2,0.2)):
    pos=[it for it in items if it[1]==1]; neg=[it for it in items if it[1]==0]
    random.shuffle(pos); random.shuffle(neg)
    def take(lst):
        n=len(lst)
        if n==0: return [],[],[]
        if n==1: return lst[:1], [], []
        if n==2: return lst[:1], [], lst[1:2]
        n_tr,n_va,n_te=_safe_split_counts(n,ratios)
        tr=lst[:n_tr]; va=lst[n_tr:n_tr+n_va]; te=lst[n_tr+n_va:n_tr+n_va+n_te]
        return tr,va,te
    trp,vap,tep=take(pos); trn,van,ten=take(neg)
    tr=trp+trn; va=vap+van; te=tep+ten
...
def groupwise_split_by_key(items: List[Tuple[str,int]], ratios=(0.6,0.2,0.2)):
    ...
    random.shuffle(keys)
    N = len(items)
    target = np.array(ratios, dtype=float); target /= target.sum()
...
class CompilerProvenanceExtractor:
    ...
    def _compute_byte_statistics(self, data: bytes) -> Dict[str, float]:
        if len(data) == 0:
            return {"entropy":0.0,"zero_ratio":0.0,"ascii_ratio":0.0,"byte_diversity":0.0,"avg_byte":0.0,"byte_std":0.0}
        ...
        return {"entropy":entropy,"zero_ratio":zero_ratio,"ascii_ratio":ascii_ratio,"byte_diversity":byte_diversity,"avg_byte":avg_byte,"byte_std":byte_std}
    ...
    def extract(self, path: str) -> Tuple[np.ndarray, List[str], str]:
        ...
        features.extend([
            stats["entropy"], stats["zero_ratio"], stats["ascii_ratio"],
            stats["byte_diversity"], stats["avg_byte"]/255.0, stats["byte_std"]/255.0
        ])
        feature_names.extend(["entropy","zero_ratio","ascii_ratio","byte_diversity","avg_byte_norm","byte_std_norm"])
        ...
        while len(features) < 512:
            feature_names.append(f"pad_{len(features)}"); features.append(0.0)
        features = features[:512]; feature_names = feature_names[:512]
...
class LegacyFeatureExtractor:
    ...
    def _legacy_elf_header(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {"elf_type":0.0,"machine_type":0.0,"entry_point":0.0,"section_header_count":0.0,"program_header_count":0.0}
        d = {}
        m = re.search(r"Type:\s+(\w+)", out); d["elf_type"] = float(len(m.group(1))) if m else 0.0
        m = re.search(r"Machine:\s+([^\n]+)", out); d["machine_type"] = float(len(m.group(1).strip())) if m else 0.0
        m = re.search(r"Entry point address:\s+(0x[0-9a-fA-F]+)", out); d["entry_point"] = 1.0 if m else 0.0
        m = re.search(r"Number of section headers:\s+(\d+)", out); d["section_header_count"] = float(int(m.group(1))) if m else 0.0
        m = re.search(r"Number of program headers:\s+(\d+)", out); d["program_header_count"] = float(int(m.group(1))) if m else 0.0
        return d

    def _legacy_symbol_table(self, p: str) -> Dict[str, Any]:
        out = self._run_legacy([self.BIN_OBJDUMP or "objdump", "-t", p])
        if not out: return {k:0.0 for k in [
            'total_symbols','function_symbols','data_symbols','local_symbols','global_symbols','weak_symbols',
            'artificial_symbols','lto_symbols','undefined_symbols','common_symbols','absolute_symbols',
            'debug_symbols','function_ratio','data_ratio','local_ratio','global_ratio','artificial_ratio','lto_ratio'
        ]}
        f = {k:0 for k in [
            'total_symbols','function_symbols','data_symbols','local_symbols','global_symbols','weak_symbols',
            'artificial_symbols','lto_symbols','undefined_symbols','common_symbols','absolute_symbols','debug_symbols'
        ]}
        for line in out.splitlines():
            if not line or "SYMBOL TABLE" in line: continue
            parts = line.split()
            ...
            if 'F' in st: f['function_symbols'] += 1
            if any(c in st for c in ("O","D","B")): f['data_symbols'] += 1
            if 'l' in sb: f['local_symbols'] += 1
            elif 'g' in sb: f['global_symbols'] += 1
            elif 'w' in sb: f['weak_symbols'] += 1
            if 'a' in st: f['absolute_symbols'] += 1
            elif 'C' in st: f['common_symbols'] += 1
            elif 'U' in st: f['undefined_symbols'] += 1
            elif 'N' in st: f['debug_symbols'] += 1
            if '<artificial>' in sn: f['artificial_symbols'] += 1
            if ('LTO' in sn) or ('lto' in sn): f['lto_symbols'] += 1
        t = f['total_symbols'] or 1
        for k in f: f[k] = float(f[k])
        f.update({
    ...
    def _legacy_sections(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {
            'total_sections':0.0,'text_sections':0.0,'data_sections':0.0,'bss_sections':0.0,'debug_sections':0.0,
            'total_text_size':0.0,'total_data_size':0.0,'total_bss_size':0.0,'total_debug_size':0.0,
            'text_size_ratio':0.0,'data_size_ratio':0.0,'bss_size_ratio':0.0
        }
        f = {'total_sections':0,'text_sections':0,'data_sections':0,'bss_sections':0,'debug_sections':0,
             'total_text_size':0,'total_data_size':0,'total_bss_size':0,'total_debug_size':0}
        for line in out.splitlines():
            if not line or 'Sections:' in line or 'Idx' in line: continue
            parts = line.split()
            ...
            try:
                ...
                size = 0
            f['total_sections'] += 1
            if '.text' in name: f['text_sections'] += 1; f['total_text_size'] += size
            elif ('.data' in name) or ('.rodata' in name): f['data_sections'] += 1; f['total_data_size'] += size
            elif '.bss' in name: f['bss_sections'] += 1; f['total_bss_size'] += size
            elif '.debug' in name or '.comment' in name: f['debug_sections'] += 1; f['total_debug_size'] += size
    ...
    def _legacy_relocs(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {
            'total_relocations':0.0,'text_relocations':0.0,'data_relocations':0.0,
            'plt_relocations':0.0,'got_relocations':0.0,'dynamic_relocations':0.0,
            'plt_ratio':0.0,'got_ratio':0.0,'dynamic_ratio':0.0
        }
        f = {'total_relocations':0,'text_relocations':0,'data_relocations':0,
             'plt_relocations':0,'got_relocations':0,'dynamic_relocations':0}
        for line in out.splitlines():
            if not line or 'RELOCATION' in line: continue
            f['total_relocations'] += 1
            if 'PLT' in line: f['plt_relocations'] += 1
            elif 'GOT' in line: f['got_relocations'] += 1
            elif 'DYNAMIC' in line: f['dynamic_relocations'] += 1
            elif '.text' in line: f['text_relocations'] += 1
            elif '.data' in line or '.rodata' in line: f['data_relocations'] += 1
    ...
    def _legacy_dynamic(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {
            'total_dynamic_entries':0.0,'needed_libraries':0.0,'rpath_entries':0.0,'runpath_entries':0.0,
            'symbol_versions':0.0,'init_functions':0.0,'fini_functions':0.0
        }
        f = {'total_dynamic_entries':0,'needed_libraries':0,'rpath_entries':0,'runpath_entries':0,
             'symbol_versions':0,'init_functions':0,'fini_functions':0}
        for line in out.splitlines():
            if not line or 'DYNAMIC' in line: continue
            f['total_dynamic_entries'] += 1
            u = line.upper()
            if 'NEEDED' in u: f['needed_libraries'] += 1
            elif 'RPATH' in u: f['rpath_entries'] += 1
            elif 'RUNPATH' in u: f['runpath_entries'] += 1
            elif 'VERSYM' in u or 'VERNEED' in u: f['symbol_versions'] += 1
            elif 'INIT' in u: f['init_functions'] += 1
            elif 'FINI' in u: f['fini_functions'] += 1
        for k in f: f[k] = float(f[k])
        return f
    ...
    def _legacy_strings(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {
            'total_strings':0.0,'lto_strings':0.0,'compiler_strings':0.0,'optimization_strings':0.0,
            'linker_strings':0.0,'gcc_strings':0.0,'gnu_strings':0.0,
            'lto_string_ratio':0.0,'compiler_string_ratio':0.0,'optimization_string_ratio':0.0
        }
        arr = out.splitlines()
        f = {'total_strings': len(arr), 'lto_strings': 0, 'compiler_strings': 0, 'optimization_strings': 0,
             'linker_strings': 0, 'gcc_strings': 0, 'gnu_strings': 0}
        for s in arr:
            sl = s.lower()
            if ('lto' in sl) or ('link-time' in sl): f['lto_strings'] += 1
            if ('gcc' in sl) or ('clang' in sl):     f['compiler_strings'] += 1
            if ('optimization' in sl) or ('optimize' in sl): f['optimization_strings'] += 1
            if ('linker' in sl) or (sl == 'ld'):     f['linker_strings'] += 1
            if 'gcc:' in sl:                         f['gcc_strings'] += 1
            if 'gnu' in sl:                          f['gnu_strings'] += 1
    ...
    def _legacy_bytes(self, p: str) -> Dict[str, Any]:
        try:
            ...
            return {'file_size':0.0,'zero_byte_ratio':0.0,'ascii_ratio':0.0,'entropy':0.0,'elf_header_present':0.0,'lto_patterns':0.0}
        if not b:
            return {'file_size':0.0,'zero_byte_ratio':0.0,'ascii_ratio':0.0,'entropy':0.0,'elf_header_present':0.0,'lto_patterns':0.0}
        arr = np.frombuffer(b, dtype=np.uint8)
        counts = np.bincount(arr, minlength=256).astype(np.float64); total = counts.sum()
        ent = 0.0
        if total > 0:
            p0 = counts/total; p0 = p0[p0>0]; ent = float(-(p0*np.log2(p0)).sum())
        ...
        return {
            ...
            'entropy': ent, 'elf_header_present': float(1.0 if b.startswith(b'\x7fELF') else 0.0),
    ...
    def _legacy_compiler(self, p: str) -> Dict[str, Any]:
        ...
        if not out: return {'has_comment_section':0.0,'has_note_section':0.0,'has_debug_sections':0.0,'compiler_info':0.0,'build_id_present':0.0}
        f = {'has_comment_section':0.0,'has_note_section':0.0,'has_debug_sections':0.0,'compiler_info':0.0,'build_id_present':0.0}
        if '.comment' in out: f['has_comment_section'] = 1.0
        if '.note' in out:    f['has_note_section'] = 1.0
        if '.debug' in out:   f['has_debug_sections'] = 1.0
        if 'GNU_BUILD_ID' in out: f['build_id_present'] = 1.0
        return f

    def _legacy_optim(self, p: str) -> Dict[str, Any]:
        out = self._run_legacy([self.BIN_OBJDUMP or "objdump","-d",p], timeout=15)
        if not out:
            return {'optimization_level':0.0,'has_inline_functions':0.0,'has_unrolled_loops':0.0,'has_vectorized_code':0.0,'has_tail_calls':0.0}
        f = {'optimization_level':0.0,'has_inline_functions':0.0,'has_unrolled_loops':0.0,'has_vectorized_code':0.0,'has_tail_calls':0.0}
        if ('callq' in out and 'nop' in out): f['has_inline_functions'] = 1.0
        if ('rep ' in out and 'mov' in out):  f['has_unrolled_loops']  = 1.0
        if ('xmm' in out) or ('ymm' in out):  f['has_vectorized_code'] = 1.0
        if ('jmp' in out and 'ret' in out):   f['has_tail_calls']      = 1.0
        return f
...
class O3FocusedFeatureExtractor(LegacyFeatureExtractor):
    ...
    def __init__(self):
        super().__init__()
        self.BIN_OBJDUMP = which_any([
            "aarch64-linux-gnu-objdump", "llvm-objdump", "objdump"
        ]) or "objdump"
    ...
    def _sec_sizes_map(self, p: str) -> Dict[str, int]:
        ...
        names = [".text",".rodata",".data",".bss",".plt",".plt.sec",".got",".got.plt",
                 ".eh_frame",".gcc_except_table",".init_array",".fini_array",
                 ".text.hot",".text.unlikely",".group",".gnu.version",".gnu.version_d",".gnu.version_r"]
        sizes = {n:0 for n in names}
        if not out: return sizes
        for line in out.splitlines():
            # 解析：] <name> ... <size_hex>
    ...
    def _dyn_relocs(self, p: str) -> Dict[str, int]:
        ...
        if not out: return rel
        for line in out.splitlines():
            if "R_AARCH64_" in line or "JUMP_SLOT" in line:
                rel["TOTAL"] += 1
                if "JUMP_SLOT" in line: rel["JUMP_SLOT"] += 1
                elif "GLOB_DAT" in line: rel["GLOB_DAT"] += 1
                elif "IRELATIVE" in line: rel["IRELATIVE"] += 1
                elif "RELATIVE" in line: rel["RELATIVE"] += 1
        return rel

    def _sym_table(self, p: str) -> Dict[str, int]:
        out = self._run_legacy([self.BIN_READELF, "-sW", p], timeout=20)
        d = {"SYM_TOTAL":0,"FUNC_TOTAL":0,"FUNC_LOCAL":0,"FUNC_GLOBAL":0,"FUNC_WEAK":0,"VIS_DEFAULT":0,"VIS_HIDDEN":0}
        if not out: return d
        for line in out.splitlines():
            ...
            if "FUNC" in line:
                d["FUNC_TOTAL"] += 1
                if " GLOBAL " in line: d["FUNC_GLOBAL"] += 1
                elif " LOCAL " in line: d["FUNC_LOCAL"]  += 1
                if " WEAK " in line: d["FUNC_WEAK"] += 1
                if " DEFAULT " in line: d["VIS_DEFAULT"] += 1
                if " HIDDEN " in line:  d["VIS_HIDDEN"]  += 1
        return d

    def _disasm_stats_aarch64(self, p: str) -> Dict[str, int]:
        out = self._run_legacy([self.BIN_OBJDUMP, "-d", p], timeout=40)
        c = {"INSN_TOTAL":0,"BL":0,"BLR":0,"BR":0,"B":0,"BCOND":0,"RET":0,
             "CBZ":0,"CBNZ":0,"TBZ":0,"TBNZ":0,"ADRP":0,"LDR":0,"ADD":0,"VEC":0}
        if not out: return c
        for line in out.splitlines():
            if ":" not in line:
                continue
            ...
            c["INSN_TOTAL"] += 1
            # 匹配注意单词边界
            if re.search(r"\bbl\b", low):   c["BL"]  += 1
            if re.search(r"\bblr\b", low):  c["BLR"] += 1
            if re.search(r"\bbr\b", low):   c["BR"]  += 1
            if re.search(r"\bb\b(?!\w)", low): c["B"] += 1
            if re.search(r"\bb\.[a-z]{2,3}\b", low): c["BCOND"] += 1
            if re.search(r"\bret\b", low):  c["RET"] += 1
            if re.search(r"\bcbz\b", low):  c["CBZ"] += 1
            if re.search(r"\bcbnz\b", low): c["CBNZ"] += 1
            if re.search(r"\btbz\b", low):  c["TBZ"] += 1
            if re.search(r"\btbnz\b", low): c["TBNZ"] += 1
            if re.search(r"\badrp\b", low): c["ADRP"] += 1
            if re.search(r"\bldr\b", low):  c["LDR"]  += 1
            if re.search(r"\badd\b", low):  c["ADD"]  += 1
    ...
    def extract(self, path: str) -> Tuple[np.ndarray, List[str], str]:
        ...
        # 汇总尺寸
        text = max(1, ss[".text"])
        ro   = ss[".rodata"]; data = ss[".data"]; bss = ss[".bss"]
        ...
        log = lambda x: float(np.log1p(max(0.0, float(x))))
...
class CompilerProvenanceSVM:
    def __init__(self, C: float = 0.1, class_weight: Optional[str] = 'balanced', random_state: int = 42,
        ...
        self.svm = LinearSVC(C=C, class_weight=class_weight, random_state=random_state, max_iter=50000, dual=False, tol=1e-4)
...
class CompilerProvenanceRF:
    def __init__(self, n_estimators: int = 400, max_depth: Optional[int] = None, random_state: int = 42):
        self.rf = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, min_samples_split=2,
                                         min_samples_leaf=1, n_jobs=-1, random_state=random_state)
        self.is_fitted = False
    def fit(self, X, y): self.rf.fit(X, y); self.is_fitted=True
    def predict(self, X): 
        if not self.is_fitted: raise ValueError("fit first"); 
        return self.rf.predict(X)
    def predict_proba(self, X):
        if not self.is_fitted: raise ValueError("fit first"); 
        return self.rf.predict_proba(X) if hasattr(self.rf,"predict_proba") else np.column_stack([1-self.rf.predict(X), self.rf.predict(X)])
    def get_feature_importance(self)->np.ndarray:
        return getattr(self.rf, "feature_importances_", np.zeros(self.rf.n_features_in_ if hasattr(self.rf,'n_features_in_') else 0))

class CompilerProvenanceGBDT:
    def __init__(self, n_estimators: int = 400, learning_rate: float = 0.05, max_depth: int = 3,
                 subsample: float = 0.9, random_state: int = 42):
        ...
        self.is_fitted=False
    def fit(self,X,y): self.gbdt.fit(X,y); self.is_fitted=True
    def predict(self,X):
        if not self.is_fitted: raise ValueError("fit first")
        return self.gbdt.predict(X)
    def predict_proba(self,X):
        if not self.is_fitted: raise ValueError("fit first")
        if hasattr(self.gbdt,"predict_proba"): return self.gbdt.predict_proba(X)
        scores = self.gbdt.decision_function(X); proba = 1/(1+np.exp(-scores))
        return np.column_stack([1-proba, proba])
    def get_feature_importance(self)->np.ndarray:
        return getattr(self.gbdt, "feature_importances_", np.zeros(self.gbdt.n_features_in_ if hasattr(self.gbdt,'n_features_in_') else 0))

class CompilerProvenanceLR:
    def __init__(self, C: float = 1.0, penalty: str = "l2", class_weight: Optional[str] = None, random_state: int = 42):
        self.scaler = StandardScaler()
        self.lr = LogisticRegression(C=C, penalty=penalty, solver="lbfgs", max_iter=2000,
                                     class_weight=class_weight, random_state=random_state)
        self.is_fitted=False
    def fit(self,X,y):
        Xs=self.scaler.fit_transform(X); self.lr.fit(Xs,y); self.is_fitted=True
    def predict(self,X):
        if not self.is_fitted: raise ValueError("fit first")
    ...
    def get_feature_importance(self)->np.ndarray:
        ...
        if coef is None: return np.zeros(self.lr.n_features_in_ if hasattr(self.lr,'n_features_in_') else 0)
        return np.abs(coef[0])

class CompilerProvenanceMLP:
    def __init__(self, hidden_layer_sizes=(256,64), activation="relu", alpha=1e-4, learning_rate_init=1e-3, max_iter=400, random_state=42):
        self.scaler = StandardScaler()
        self.mlp = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation=activation, alpha=alpha,
                                 learning_rate_init=learning_rate_init, max_iter=max_iter, random_state=random_state,
                                 early_stopping=True, validation_fraction=0.1)
        self.is_fitted=False
    def fit(self,X,y): 
        Xs=self.scaler.fit_transform(X); self.mlp.fit(Xs,y); self.is_fitted=True
    def predict(self,X):
        if not self.is_fitted: raise ValueError("fit first")
...
class _BaseTrainer:
    ...
    def _collect_files(self) -> List[Tuple[str,int]]:
        ...
        out=[]
        if base_non.is_dir():  out += [(str(p),0) for p in base_non.rglob("*.so")]
        else: self.logger.warning(f"输入目录不存在: {base_non}")
        if base_full.is_dir(): out += [(str(p),1) for p in base_full.rglob("*.so")]
        else: self.logger.warning(f"输入目录不存在: {base_full}")
        if base_thin.is_dir(): out += [(str(p),1) for p in base_thin.rglob("*.so")]
        else: self.logger.warning(f"输入目录不存在: {base_thin}")
        return out
...
class LegacyFeatureTrainer(_BaseTrainer):
    ...
    def run(self):
        try:
            ...
            grid = GridSearchCV(base_svm, param_grid, cv=StratifiedKFold(n_splits=5, shuffle=True, random_state=42),
                                scoring='precision', n_jobs=-1, verbose=0)
            grid.fit(Xtr,ytr); best_params = grid.best_params_
            ...
            # ---- RF ----
            rf_space = [{"n_estimators":300,"max_depth":None},{"n_estimators":600,"max_depth":None},
                        {"n_estimators":400,"max_depth":20},{"n_estimators":400,"max_depth":40}]
            best_rf, best_rf_prec, best_rf_params = None, -1.0, None
            for ps in rf_space:
                mdl = CompilerProvenanceRF(**ps); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_rf_prec: best_rf_prec, best_rf, best_rf_params = prec, mdl, ps
            rf = best_rf
            va_proba = rf.predict_proba(Xva); thr_rf,_ = optimize_threshold(yva, va_proba, metric='precision')
            te_pred_rf = (rf.predict_proba(Xte)[:,1] >= thr_rf).astype(int)

            # ---- GBDT ----
            gbdt_space = [{"n_estimators":300,"learning_rate":0.05,"max_depth":3,"subsample":0.9},
            ...
            best_g, best_g_prec, best_g_params = None, -1.0, None
            for ps in gbdt_space:
                mdl = CompilerProvenanceGBDT(**ps); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_g_prec: best_g_prec, best_g, best_g_params = prec, mdl, ps
            gbdt = best_g
            va_proba = gbdt.predict_proba(Xva); thr_g,_ = optimize_threshold(yva, va_proba, metric='precision')
            te_pred_g = (gbdt.predict_proba(Xte)[:,1] >= thr_g).astype(int)

            # ---- LR ----
            lr_space = [(0.5,None),(1.0,None),(2.0,None),(1.0,'balanced'),(2.0,'balanced')]
            best_lr, best_lr_prec, best_lr_cfg = None, -1.0, None
            for C, cw in lr_space:
                mdl = CompilerProvenanceLR(C=C, penalty="l2", class_weight=cw); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_lr_prec: best_lr_prec, best_lr, best_lr_cfg = prec, mdl, (C, cw)
            lr = best_lr
            va_proba = lr.predict_proba(Xva); thr_lr,_ = optimize_threshold(yva, va_proba, metric='precision')
            te_pred_lr = (lr.predict_proba(Xte)[:,1] >= thr_lr).astype(int)

            # ---- MLP ----
            mlp_space = [(128,), (256,64), (256,128,64)]
            best_mlp, best_mlp_prec, best_mlp_cfg = None, -1.0, None
            for hl in mlp_space:
                mdl = CompilerProvenanceMLP(hidden_layer_sizes=hl); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_mlp_prec: best_mlp_prec, best_mlp, best_mlp_cfg = prec, mdl, hl
            mlp = best_mlp
            va_proba = mlp.predict_proba(Xva); thr_mlp,_ = optimize_threshold(yva, va_proba, metric='precision')
            ...
            lines = ["="*72,"Legacy特征 LTO 检测报告（group_split=%s）"%self.group_split,"="*72]
            for k,v in res.items():
                lines.append(f"{k}: Prec={v['precision']:.4f} F1={v['f1']:.4f} Acc={v['accuracy']:.4f} Rec={v['recall']:.4f}")
            with open(self.out_dir/"report.txt","w",encoding="utf-8") as f: f.write("\n".join(lines))
            print("\n".join(lines))
            return True
        except Exception as e:
...
class CompilerProvenanceTrainer(_BaseTrainer):
    ...
    def _feature_group_of(name: str) -> str:
        if name.startswith("byte_freq_"): return "LOW"
        if name.startswith("file_size") or name.startswith("percentile_"): return "LOW"
        if name.startswith("byte_pair_"): return "MID"
        if name in ("entropy","zero_ratio","ascii_ratio","byte_diversity","avg_byte_norm","byte_std_norm"): return "MID"
        return "NOISE"

    def _select_hierarchical_features(self, names: List[str], X: np.ndarray, y: np.ndarray) -> List[int]:
        try:
        ...
        for i, n in enumerate(names): groups_static[self._feature_group_of(n)].append(i)
        low_idx = groups_static.get("LOW", []); mid_idx = groups_static.get("MID", [])
        ...
        if not final: final = list(range(min(K, X.shape[1])))
        return final
    ...
    def prepare(self):
        ...
        try:
            ...
            if 0 < len(sel_idx) < len(trds.feature_names):
                trds.X = trds.X[:, sel_idx]; vads.X = vads.X[:, sel_idx]; teds.X = teds.X[:, sel_idx]
    ...
    def run(self):
        try:
            ...
            lines = ["="*72,"hier特征 LTO 检测报告（group_split=%s）"%self.group_split,"="*72]
            lines.append(f"SVM: Prec={res['precision']:.4f} F1={res['f1']:.4f} Acc={res['accuracy']:.4f} Rec={res['recall']:.4f}")
            with open(self.out_dir/"report.txt","w",encoding="utf-8") as f: f.write("\n".join(lines))
            print("\n".join(lines))
            return True
        except Exception as e:
...
class O3FeatureTrainer(LegacyFeatureTrainer):
    ...
    def prepare(self):
        ...
        vads = LegacyFeatureDataset(va, extractor)
        teds = LegacyFeatureDataset(te, extractor)
        self.logger.info(f"[O3] 特征维度: {len(trds.feature_names)} | 训练:{len(trds)} 验证:{len(vads)} 测试:{len(teds)}")
        # 记录特征名便于诊断
        try:
            with open(self.out_dir/"o3_feature_names.txt","w",encoding="utf-8") as f:
                for n in trds.feature_names: f.write(n+"\n")
        except Exception:
            pass
        return trds, vads, teds

# -------------------- Hybrid（新） --------------------
def _select_hier_features_generic(names: List[str], X: np.ndarray, y: np.ndarray, K: int) -> List[int]:
    """
    仅对 HIER 子向量做层次选择；names 为去前缀后的原始 hier 特征名（如 byte_freq_0 等）
    返回的是**局部**索引（相对 X 的列）。
    """
    def group_of(n: str) -> str:
        if n.startswith("byte_freq_"): return "LOW"
        if n.startswith("file_size") or n.startswith("percentile_"): return "LOW"
        if n.startswith("byte_pair_"): return "MID"
        if n in ("entropy","zero_ratio","ascii_ratio","byte_diversity","avg_byte_norm","byte_std_norm"): return "MID"
        return "NOISE"
    try:
        from sklearn.feature_selection import f_classif
        f_vals, _ = f_classif(X, y)
    ...
    for i, n in enumerate(names): groups[group_of(n)].append(i)
    low_idx = groups.get("LOW", []); mid_idx = groups.get("MID", [])
    pool_lm = low_idx + mid_idx
    high_pool: List[int] = []
    if pool_lm:
        k_high = max(1, int(np.ceil(0.10 * len(pool_lm))))
...
class HybridFeatureTrainer(_BaseTrainer):
    ...
    混合特征训练器：
      ...
      - 训练全套 6 模型 + 校准 + 阈值优化
    """
    def __init__(self, level: str, log_dir: str = "outs", group_split: bool = True, hybrid_hier_max_features: int = 128):
    ...
    def prepare(self):
        ...
        self.logger.info(f"[Hybrid] 初始维度: {len(trds.feature_names)} | 训练:{len(trds)} 验证:{len(vads)} 测试:{len(teds)}")
        ...
        try:
            ...
            if self.hier_k > 0 and len(hier_idx_tr) > 0:
                ...
                keep = set([i for i in range(len(trds.feature_names)) if not trds.feature_names[i].startswith("HIER_")] + sel_global)
    ...
    def run(self):
        try:
            ...
            for ps in rf_space:
                mdl = CompilerProvenanceRF(**ps); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_rf_prec: best_rf_prec, best_rf = prec, mdl
            rf = best_rf
            ...
            for ps in gbdt_space:
                mdl = CompilerProvenanceGBDT(**ps); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_g_prec: best_g_prec, best_g = prec, mdl
            gbdt = best_g
            ...
            for C, cw in lr_space:
                mdl = CompilerProvenanceLR(C=C, penalty="l2", class_weight=cw); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_lr_prec: best_lr_prec, best_lr = prec, mdl
            lr = best_lr
            ...
            for hl in mlp_space:
                mdl = CompilerProvenanceMLP(hidden_layer_sizes=hl); mdl.fit(Xtr,ytr)
                pred = mdl.predict(Xva); prec = precision_score(yva, pred, zero_division=0)
                if prec > best_mlp_prec: best_mlp_prec, best_mlp = prec, mdl
            mlp = best_mlp
            ...
            lines = ["="*72,"Hybrid特征 LTO 检测报告（group_split=%s）"%self.group_split,"="*72]
            for k,v in res.items():
                lines.append(f"{k}: Prec={v['precision']:.4f} F1={v['f1']:.4f} Acc={v['accuracy']:.4f} Rec={v['recall']:.4f}")
            with open(self.out_dir/"report.txt","w",encoding="utf-8") as f: f.write("\n".join(lines))
            print("\n".join(lines))
...
def main(args):
    ...
    if args.feature_mode == "hier":
        trainer = CompilerProvenanceTrainer(level=args.level, hier_max_features=args.hier_max_features, group_split=args.group_split)
        ...
        trainer = HybridFeatureTrainer(level=args.level, group_split=args.group_split, hybrid_hier_max_features=args.hybrid_hier_max_features)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--feature-mode", choices=["default","hier","o3","hybrid"], default="hybrid",
    ...
    ap.add_argument("--group-split", action="store_false", help="按库 key 分组切分，避免泄漏（开=跨库泛化，关=库内泛化）")
    args = ap.parse_args()
    main(args)
