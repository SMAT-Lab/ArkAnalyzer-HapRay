...
class ResourceUsage_PerformanceDynamic_ArkTsExample_0030(PerfTestCase):
    ...
    def process(self):
        ...
        self.execute_performance_step("ArkTsExample-组件嵌套测试场景-step1 10layers", 5, step,
                                      sample_all_processes=True)
        self.driver.swipe_to_back()

        self.touch_by_text('NestedLayerPage', 2)
        self.driver.input_text(BY.type('TextInput'), '50')
        self.execute_performance_step("ArkTsExample-组件嵌套测试场景-step2 50layers", 5, step,
                                      sample_all_processes=True)
        self.driver.swipe_to_back()

        self.touch_by_text('NestedLayerPage', 2)
        self.driver.input_text(BY.type('TextInput'), '100')
        self.execute_performance_step("ArkTsExample-组件嵌套测试场景-step3 100layers", 5, step,
                                      sample_all_processes=True)
        self.driver.swipe_to_back()

        self.touch_by_text('NestedLayerPage', 2)
        self.driver.input_text(BY.type('TextInput'), '500')
        self.execute_performance_step("ArkTsExample-组件嵌套测试场景-step4 500layers", 5, step,
                                      sample_all_processes=True)
        self.driver.swipe_to_back()
