...
def _run(cmd, cwd=None, env=None):
    ...
    res = subprocess.run(
        cmd, check=False, cwd=cwd, env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    if res.returncode != 0:
...
def integrate_four(in_dir: str, out_path: str):
    with open(os.path.join(in_dir, 'fileDepGraph.json'), encoding='utf-8') as f:
        fileDepGraph_data = json.load(f)
    with open(os.path.join(in_dir, 'moduleDepGraph.json'), encoding='utf-8') as f:
        moduleDepGraph_data = json.load(f)
    with open(os.path.join(in_dir, 'hapray_report.json'), encoding='utf-8') as f:
        hapray_report_data = json.load(f)
