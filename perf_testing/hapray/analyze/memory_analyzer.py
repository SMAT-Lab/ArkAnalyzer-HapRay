...
class MemoryAnalyzer(BaseAnalyzer):
    ...
    def _create_record_models(
        ...
        for record in records:
            ...
            # componentCategory 是枚举值（0-8, -1），直接存储为 INTEGER，不通过字典表
            component_category = record.get('componentCategory')
            component_category_id = (
                component_category
                if isinstance(component_category, int)
                else (int(component_category) if component_category is not None else -1)
            )
            ...
            record_model = MemoryRecordStorageModel(
                ...
                componentCategory=component_category_id,
