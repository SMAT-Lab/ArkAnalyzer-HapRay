...
class MemoryCallchainModel(BaseModel):
    """Memory analysis callchain frame model"""
...
class MemoryAnalyzer(BaseAnalyzer):
    ...
    def _save_results_to_db(self):
        ...
        try:
            ...
            self.create_model_table(MemoryCallchainModel)
    ...
    def _create_memory_callchains_indexes(self):
        ...
        self.create_index('idx_memory_callchains_callchainId', 'memory_callchains', 'callchainId')
        self.create_index('idx_memory_callchains_depth', 'memory_callchains', 'depth')
    ...
    def _save_step_data_to_db(self, step_id: int, step_data: dict):
        ...
        self.exec_sql('DELETE FROM memory_callchains WHERE step_id = ?', (step_id,))
    ...
    def _create_callchain_models(self, callchains: dict[int, list[dict]]) -> list[MemoryCallchainModel]:
        """Create MemoryCallchainModel instances from callchain dictionaries

        Args:
            callchains: Mapping of callchainId to frame list

        Returns:
            List of MemoryCallchainModel instances
        """
        models: list[MemoryCallchainModel] = []
