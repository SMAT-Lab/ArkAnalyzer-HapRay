...
从用户源码目录中的 *.callgraph.json 构建调用图，
...
class CallgraphTraverser:
    ...
    def find_entry_chains(self, symbol_name: str, max_chains: int = 5) -> list[CallChain]:
        """
    ...
    def enrich_proc_source_hints(self, hints: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        为每个 proc_source_hint 的 source_candidates 补充 call_chains 字段。
        """
        self._ensure_loaded()
        for hint in hints:
            for cand in hint.get('source_candidates') or []:
    ...
    def format_chains_for_prompt(self, hints: list[dict[str, Any]]) -> str:
        ...
        lines: list[str] = []
        for hint in hints[:5]:
            for cand in (hint.get('source_candidates') or [])[:2]:
