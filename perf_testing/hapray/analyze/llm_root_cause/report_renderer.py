...
生成 root_cause_evidence.md：规则引擎提取的采样事实 +（有源码树时）路径对齐的代码片段。
...
from .proc_source_match import pick_aligned_candidates
...
class EvidenceReportRenderer:
    ...
    def render(self, evidence: dict[str, Any]) -> str:
        ...
        lines.append('')
        lines.append('> 本文件由规则引擎生成：采样事实 + 源码片段（路径与 /proc 命中对齐时）。')
        ...
        self._append_source_scope_notice(lines, evidence)
        self._append_source_snippets(lines, proc_source_hints)
        ...
        if ui_snapshot_hints:
            ...
            for item in ui_snapshot_hints[:8]:
                lines.append(
                    f'- `{item.get("name", "unknown")}` [{item.get("kind", "unknown")}] count={item.get("count", 0)}'
                )
            lines.append('')
            ...
            for i, frame in enumerate(representative_frames[:3], 1):
                ...
                if proc_hits:
                    ...
                    for h in proc_hits[:5]:
                        ...
                        short = '/'.join(p.parts[-2:]) if p.name.lower() == 'index.ts' and len(p.parts) >= 2 else p.name
                        hit_strs.append(f'{short}:{h.get("line")}::{h.get("symbol_name", "?")} [{h.get("via", "?")}]')
    ...
    def _append_source_scope_notice(
        self,
        lines: list[str],
        evidence: dict[str, Any],
    ) -> None:
        """Explain when user-provided source tree is a small subset vs /proc runtime paths."""
        proc_hints = evidence.get('proc_source_hints') or []
        if not proc_hints:
            return

        stats = evidence.get('source_stats') or {}
        file_count = int(stats.get('file_count') or 0)
        input_root = str(stats.get('input_root') or '')
        top_dirs = stats.get('top_level_dirs') or []

        proc_samples = [str(h.get('source_path') or '') for h in proc_hints[:5] if h.get('source_path')]
        if not proc_samples:
            return

        # Heuristic: /proc under src/main/ets/... but source tree is a tiny taobao_main-only pack
        only_taobao_main = (
            file_count > 0
            and file_count < 200
            and (not top_dirs or top_dirs in (['taobao_main'], ['index', 'taobao_main']))
        )
        if not only_taobao_main and file_count >= 200:
            return

        lines.append('## 源码输入范围说明（重要）')
        lines.append('')
        if file_count:
            lines.append(
            ...
            lines.append('- 已配置源码目录，但未读取到 `index/stats.json` 统计')
        lines.append('- 运行时 `/proc` 命中路径来自**正在运行的完整应用**，例如：')
        for p in proc_samples[:4]:
            lines.append(f'  - `{p}`')
        lines.append(
            '- 若源码树只有 `taobao_main/` 等子包，**不包含**上述路径对应文件，'
            '则「有源码目录」仍会出现**无匹配片段**——不是工具未搜索，而是**离线树未覆盖运行时模块**。'
        )
        lines.append(
            '- 解决办法：提供**完整主模块**源码（含 `infoflow/`、`Util.ts` 等），补全 bundle 源码后重新 `update`。'
        )
        lines.append('')

    def _append_source_snippets(
        self,
        lines: list[str],
        ...
        for hint in proc_source_hints[:8]:
            ...
            if isinstance(direct, dict) and direct.get('code_snippet'):
                ...
                blocks.extend(
                    self._format_snippet_block(
                        title=f'直接命中 {loc}',
                        candidate=direct,
                        match_note='按 /proc 路径在源码树定位',
                    )
                )
                continue

            aligned = [
                c for c in pick_aligned_candidates(hint, hint.get('source_candidates') or []) if c.get('code_snippet')
            ]
            if aligned:
                has_any_snippet = True
                blocks.extend(
            ...
            blocks.append(f'- {loc} — **无匹配源码片段**（源码索引/树中未包含该路径，hits={hint.get("hit_count", 0)}）')

        if not has_any_snippet and not blocks:
            return

        lines.append('## 源码片段（与 /proc 命中关联）')
        lines.append('')
        if not has_any_snippet:
            lines.append(
                '> 已配置源码目录，但当前 Top 命中路径（如 infoflow 动态模块）未落入离线源码范围；以下仅列出未命中项。'
            )
            lines.append('')
        lines.extend(blocks)
        lines.append('')
