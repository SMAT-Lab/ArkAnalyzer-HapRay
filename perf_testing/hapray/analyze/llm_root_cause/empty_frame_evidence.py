...
class EmptyFrameEvidenceExtractor:
    ...
    def _normalize_frame(self, frame: dict[str, Any]) -> dict[str, Any]:
        sample_callchains = (
            frame.get('sample_callchains', []) if isinstance(frame.get('sample_callchains'), list) else []
        )
    ...
    def _collect_symbol_hints(callchains: list[dict[str, Any]]) -> list[str]:
        ...
        for callchain in callchains:
            for symbol in callchain.get('symbols', []):
                if (
                    any(hint in symbol for hint in _VSYNC_SYMBOL_HINTS + _JS_LOOP_SYMBOL_HINTS)
                    and symbol not in ordered
                ):
    ...
    def _extract_ui_snapshot_hints(self, step_id: str) -> list[dict[str, Any]]:
        ...
        for path in sorted(ui_root.glob('element_tree_*.txt')) + sorted(ui_root.glob('inspector_*.json')):
            ...
            for matched_page in _PAGE_NAME_RE.findall(text):
                page_name = matched_page.strip()
    ...
    def _collect_proc_source_hints(
        self, step_id: str, representative_frames: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        ...
        try:
            for frame in representative_frames:
                ...
                for callchain in frame.get('sample_callchains', [])[:3]:
                    ...
                    for callchain_id, delta_ns_raw, event_count in sample_rows:
                        delta_ns = self._safe_int(delta_ns_raw)
    ...
    def _build_caveats(
        ...
        if not ui_snapshot_hints:
            caveats.append('当前样本没有可用的 UI snapshot/inspector 命名线索，代码定位会更依赖源码索引模糊匹配。')
        if not proc_source_hints:
            caveats.append(
                '当前样本未命中可用的 /proc 用户态源码符号，无法直接缩到具体 ArkTS/JS 源码行，只能依赖 UI 快照和索引规则。'
            )
