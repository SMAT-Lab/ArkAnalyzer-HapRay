"""
基于规则的应用源码 / ArkTS 索引构建脚本。

输入：HarmonyOS 源码树或用户提供的解析目录（递归扫描 *.ts / *.ets）
输出：
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='构建 TS / ArkTS ETS 的规则索引')
    parser.add_argument('--input', required=True, help='源码目录，递归扫描 *.ts / *.ets')
...
def trim_module_tokens(tokens: list[str], symbol_name: str) -> list[str]:
    ...
    while trimmed and trimmed[-1].startswith('#'):
        trimmed.pop()
    if trimmed and trimmed[-1] == symbol_name or trimmed and looks_like_symbol_token(trimmed[-1]):
        trimmed.pop()
    return trimmed
...
def scan_file(file_path: Path, input_root: Path, symbol_writer: TextIO, ui_writer: TextIO) -> dict:
    ...
    return {
        ...
        'ui_keywords_top': [
            {'name': name, 'count': count} for name, count in file_summary['ui_counts'].most_common(20)
        ],
    }
...
def build_index(input_root: Path, output_dir: Path) -> dict:
    ...
    with (
        symbol_index_path.open('w', encoding='utf-8') as symbol_writer,
        ui_index_path.open('w', encoding='utf-8') as ui_writer,
    ):
