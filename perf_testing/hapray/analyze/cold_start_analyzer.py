...
class ColdStartAnalyzer(BaseAnalyzer):
    ...
    def _analyze_impl(self, step_dir: str, trace_db_path: str, perf_db_path: str) -> Dict[str, Any] | None:
        ...
        if not bundle_name:
            logging.warning(f"Could not extract bundle name from testInfo.json")
            return None
    ...
    def _parse_redundant_file(self, file_path: str) -> Dict[str, Any] | None:
        ...
        try:
            ...
            # 解析Summary信息
            summary_match = re.search(r'<----Summary---->\s*Total file number:\s*(\d+),\s*total time:\s*(\d+)ms,\s*including used file:(\d+),\s*cost time:\s*(\d+)ms,\s*and unused file:\s*(\d+),\s*cost time:\s*(\d+)ms', content)
