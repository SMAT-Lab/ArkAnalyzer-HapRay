...
CONFIG"""
    ...
    def _transfer_memory_data(self, remote_path: str, local_path: str):
        ...
        已废弃：memory 数据已包含在 trace.htrace 中，不再需要单独传输
        保留此方法用于向后兼容

        @deprecated Memory data is now included in trace.htrace
        """
        Log.warning('_transfer_memory_data is deprecated. Memory data is now included in trace.htrace')
