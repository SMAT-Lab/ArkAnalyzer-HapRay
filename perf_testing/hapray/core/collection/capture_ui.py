...
class CaptureUI:
    ...
    def capture_page(self, step_id: int, report_path: str, page_idx: int, animate: bool = False) -> dict[str, str]:
        ...
        inspector_path = os.path.join(ui_step_dir, f'inspector_page{page_idx}.json')
        self.uitree.dump_to_file(inspector_path)
        result_files['inspector'] = inspector_path

        # 1. 截屏
        screenshot_path = self._capture_screenshot(ui_step_dir, f'page{page_idx}')
        if screenshot_path:
            result_files['screenshot'] = screenshot_path

        # 2. dump element树
        element_tree_path = self._dump_page_element_tree(ui_step_dir, f'page{page_idx}')
        if element_tree_path:
            result_files['element_tree'] = element_tree_path

        if animate:
            # 3. 第2次截屏
            screenshot_path_2 = self._capture_screenshot(ui_step_dir, f'page{page_idx}_2')
            if screenshot_path_2:
                result_files['screenshot_2'] = screenshot_path_2

            # 4. 第2次dump element树
            element_tree_path_2 = self._dump_page_element_tree(ui_step_dir, f'page{page_idx}_2')
