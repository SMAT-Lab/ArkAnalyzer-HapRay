...
# from .frame_data_advanced_accessor import FrameDbAdvancedAccessor  # 未使�?
...
class EmptyFrameAnalyzer:
    ...
    def analyze_empty_frames(
        ...
        参数:
        - trace_db_path: str，trace数据库文件路�?
        - perf_db_path: str，perf数据库文件路�?
        - app_pids: list，应用进程ID列表
        - scene_dir: str，场景目录路径，用于更新缓存
        - step_id: str，步骤ID，用于更新缓�?
        返回:
        - dict，包含分析结�?
        """
        # 验证数据库兼容�?
        ...
        try:
            ...
            if step_id:
                ...
                # logging.info("预加载数据结�? %s", preload_result)
                # 确保帧负载数据缓存已初始�?
            ...
            if total_load_result.empty or total_load_result['total_load'].iloc[0] is None or pd.isna(
                ...
                total_load = 0
                logging.warning("perf_sample表中没有找到有效的event_count数据，设置total_load�?")
            else:
                total_load = total_load_result['total_load'].iloc[0]
                if total_load <= 0:
                    logging.warning("total_load计算结果�?或负�? %s，设置total_load�?", total_load)
                    total_load = 0
            # 获取所有perf样本
            ...
            if step_id:
                try:
                    ...
                    # 如果获取缓存失败，则使用空帧中的最小时间戳作为备�?
                    first_frame_time = int(trace_df['ts'].min()) if not trace_df.empty else 0
            else:
                # 如果无法获取step_id，则使用空帧中的最小时间戳作为备�?
                first_frame_time = int(trace_df['ts'].min()) if not trace_df.empty else 0
            # 初始化结果列�?
            frame_loads = []
            empty_frame_load = 0  # 主线程空帧总负载（即空刷主线程负载�?
            background_thread_load = 0  # 后台线程空帧总负�?
            # 对每个帧进行分析 - 优先使用缓存中的帧负载数�?
            for _, frame in trace_df.iterrows():
                # 构建帧标识符用于查找缓存（已注释，避免未使用变量警告）
                # _frame_key = f"{frame['ts']}_{frame['dur']}_{frame['tid']}"
                # 检查缓存中是否已有该帧的负载数�?
                ...
                if cached_frame:
                    ...
                    # 如果缓存中没有sample_callchains，尝试补�?
                    if not sample_callchains:
                        # logging.debug("缓存中缺少sample_callchains，尝试补�? ts=%s", frame['ts'])
                    ...
                    try:
                        frame_load, sample_callchains = self.load_calculator.analyze_single_frame(
                            frame, perf_df, perf_conn, step_id)
                        # logging.debug("执行帧负载分�? ts=%s, load=%s", frame['ts'], frame_load)
                    except Exception as e:
                        # 如果分析失败，使用默认�?
                        frame_load = 0
                        sample_callchains = []
                        logging.warning("执行帧负载分析失�? ts=%s, error=%s", frame['ts'], str(e))
            ...
            if not result_df.empty:
                # 分别获取主线程和后台线程的top5�?
                main_thread_frames = result_df[result_df['is_main_thread'] == 1].sort_values(
                    'frame_load', ascending=False
                ).head(5)
                ...
                if total_load > 0:
                    ...
                    logging.warning("total_load�?，无法计算百分比，设置为0")
                # 构建结果字典
                return {
                    "status": "success",
                    "summary": {
            ...
            logging.error("分析空帧时发生异�? %s", str(e))
            return None
        finally:
            trace_conn.close()
            perf_conn.close()
        ...
        for _, frame in trace_df.iterrows():
            ...
            if cached_frame:
                ...
                try:
                    ...
                    frame_load = 0
                    sample_callchains = []
                    logging.warning("帧负载分析失�? ts=%s, error=%s", frame['ts'], str(e))
            frame_loads.append({
                'ts': frame['ts'],
                'dur': frame['dur'],
                'frame_load': frame_load,
