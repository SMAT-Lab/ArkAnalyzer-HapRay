...
class FlutterEmptyFrameDetector(FrameworkSpecificDetector):
        ...
        try:
            ...
            for event_item in setpresent_events:
                ...
                frame_data = self._build_frame_data(
                    ...
                    raster_itid, raster_tid, ui_itid, ui_tid,
                    app_ipid, app_pid, app_name,
                    matched_frame, beginframe
                )
            ...
            if total_setpresent_count > 0:
                logger.info(f'其中 {self.total_detected_count} 个是空刷帧（frame_damage为空），{total_setpresent_count - self.total_detected_count} 个有脏区（非空刷，已跳过）')
            
            if not empty_frames:
                logger.info('Flutter 检测器：未检测到空刷帧（在合并去重之前）')
                return pd.DataFrame()
            
            frames_df = pd.DataFrame(empty_frames)
    ...
    def _find_flutter_process_and_threads(self) -> Optional[Tuple]:
        ...
        if self.app_pids:
            cursor.execute("""
                ...
                LIMIT 1
            """.format(','.join('?' * len(self.app_pids))), self.app_pids)
        else:
            cursor.execute("""
                SELECT p.ipid, p.pid, p.name
                FROM process p
    ...
    def _build_frame_data(
        ...
        # 判断是否主线程（线程名 == 进程名）
        is_main_thread = 1 if '1.ui' == app_name or '1.raster' == app_name else 0
    ...
    for framework_type in framework_types:
        try:
            ...
            if detector:
                ...
                # 输出检测结果
                if detected_count > 0:
                    logger.info(f'{framework_type} 框架检测器：检测到 {detected_count} 个空刷帧（frame_damage为空，在合并去重之前，可能与 flag=2 或 RS skip 重叠）')
                else:
