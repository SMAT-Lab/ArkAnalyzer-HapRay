...
class FrameDbBasicAccessor:
    ...
    def _standardize_callstack_data(callstack_df: pd.DataFrame) -> pd.DataFrame:
        ...
        callstack_df['is_distributed_call'] = (callstack_df['chainId'].notna() & (callstack_df['chainId'] != '')).astype(int)
    ...
    def _standardize_perf_sample_data(perf_df: pd.DataFrame) -> pd.DataFrame:
        ...
        perf_df['is_other_state'] = ((perf_df['thread_state'] != 'Running') & (perf_df['thread_state'] != 'Suspend')).astype(int)
    ...
    def _standardize_perf_callchain_data(callchain_df: pd.DataFrame) -> pd.DataFrame:
        ...
        callchain_df['is_leaf_level'] = (callchain_df['depth'] == callchain_df.groupby('callchain_id')['depth'].transform('max')).astype(int)
