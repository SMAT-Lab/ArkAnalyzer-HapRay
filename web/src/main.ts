...
/**
 * Placeholder strings that indicate data is not yet injected
 */
const PLACEHOLDERS = {
    DB_DATA: 'DB_DATA_PLACEHOLDER',
} as const;
...
function loadDatabaseData(): void {
    ...
    if (dbData) {
        console.log(`[Main] Database data loaded (length: ${dbData.length})`);
    } else {
...
function isDatabaseDataPlaceholder(dbData: string): boolean {
    return !dbData || dbData.trim().length === PLACEHOLDERS.DB_DATA.length || dbData.trim().length === 0;
}
