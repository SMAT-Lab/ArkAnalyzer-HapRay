...
class ToolExecutor:
    ...
    def execute_python_tool(
        ...
        """
        try:
            tool_config = self.config.get_plugin_config(plugin_name)
    ...
    def cancel_task(self, plugin_name: str) -> bool:
        """取消正在执行的任务"""
        if plugin_name in self.running_tasks:
            process = self.running_tasks[plugin_name]
            try:
                process.terminate()
    ...
    def is_running(self, plugin_name: str) -> bool:
        """检查任务是否正在运行"""
        return plugin_name in self.running_tasks
